...
const ModalOverlay = (props) =>{
  return(
    ...
    <Card className="w-[36rem] p-0 z-20">
      <header className='bg-red-700 p-4 text-white rounded-t-xl text-xl'>{props.title}</header>
      <section className='p-4'>{props.message}</section>
      <footer  className="p-4 flex justify-end">
        <Button className="rounded-xl w-32" onClick={props.onConfirm}>Tamam</Button>
      </footer>
    </Card>
    </div>
  )
...
useEffect(() =>{
  ...
  return (
     ...
     {ReactDOM.createPortal(<ModalOverlay message={message} title={title} onConfirm={onConfirm}/>,document.getElementById("overlay-root"))}
